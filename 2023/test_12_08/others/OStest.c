...
struct task_struct *ready = NULL, *run = NULL, *finish = NULL; // 定义三个队列，就绪队列，执行队列和完成队列
int num;
void GetFirst();            // 从就绪队列取得第一个节点
void Output();              // 输出队列信息
void InsertPrio(struct task_struct *in);   // 创建优先级队列，规定优先数越小，优先级越高
void InsertTime(struct task_struct *in);   // 时间片队列
void InsertFinish(struct task_struct *in); // 时间片队列
...
void InsertPrio(struct task_struct *in) // 创建优先级队列，规定优先数越小，优先级越低
{
    struct task_struct *fst, *nxt;
    fst = nxt = ready;
...
void InsertTime(struct task_struct *in) // 将进程插入到就绪队列尾部
{
    struct task_struct *fst;
    fst = ready;
...
void InsertFinish(struct task_struct *in) // 将进程插入到完成队列尾部
{
    struct task_struct *fst;
    fst = finish;
...
void PrioCreate() // 优先级调度输入函数
{
    ...
    for (i = 0; i < num; i++)
    {
        if ((tmp = (struct task_struct *)malloc(sizeof(struct task_struct))) == NULL)
        {
            perror("malloc");
            exit(1);
        }
